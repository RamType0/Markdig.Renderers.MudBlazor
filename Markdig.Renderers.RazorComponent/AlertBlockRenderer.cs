...
{
    public Action<RazorComponentRenderer, StringSlice> RenderKind { get; set; } = DefaultRenderKind;
    protected override void Write(RazorComponentRenderer renderer, AlertBlock obj)
    {
        ...
        builder.OpenRegion(0);
        {
            builder.OpenElement(0, "div");
            {
                ...
                builder.CloseRegion();
                builder.AddImplicitParagraph(3, true, false, builder =>
                {
    ...
    public static void DefaultRenderKind(RazorComponentRenderer renderer, StringSlice kind)
    {
        Span<char> span = stackalloc char[kind.Length];
        kind.AsSpan().ToUpperInvariant(span);
        var markupContent = span switch
        {
        ...
        renderer.Builder.AddMarkupContent(0, markupContent);
    }
}
